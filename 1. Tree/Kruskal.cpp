...
int find(int u) {
    if (root[u] != u) return find(root[u]);
    return root[u];
}
...
void merge(int u, int v) {
    ...
    v = find(v);
    if (u == v) return;
    if (_rank[u] < _rank[v]) root[u] = v;
    else if(_rank[u] > _rank[v]) root[v] = u;
    else {
        _rank[u]++;
        root[v] = u;
    }
...
void Kruskal() {
    ...
    if (t.size() < n - 1) cout << "Cannot find" << endl;
    else {
        cout << "Result:" << endl;
        for (int i = 0; i < t.size(); i++) cout << "        " << Path[t[i]].u << " " << Path[t[i]].v << endl;
    }
}
